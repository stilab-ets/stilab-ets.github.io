...
describe('Header.vue', () => {
  ...
  it('renders all main child components', () => {
    ...
    expect(wrapper.findComponent({ name: 'MobileMenuToggle' }).exists()).toBe(true)
    expect(wrapper.findComponent({ name: 'LanguageToggle' }).exists()).toBe(true)
  ...
  it('emits "setCurrentPage" when NavItems emits "navigate"', async () => {
    ...
    await wrapper.findComponent({ name: 'NavItems' }).vm.$emit('navigate', 'about')
  ...
  it('emits "setCurrentPage" when LogoButton emits "navigate"', async () => {
    ...
    await wrapper.findComponent({ name: 'LogoButton' }).vm.$emit('navigate', 'home')
  ...
  it('emits "languageChanged" when LanguageToggle emits "language-changed"', async () => {
    ...
    await wrapper.findComponent({ name: 'LanguageToggle' }).vm.$emit('language-changed', 'en')
