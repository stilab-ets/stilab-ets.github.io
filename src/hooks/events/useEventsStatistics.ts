...
export function useEventsStatistics(events: Ref<Event[]>): UseEventsStatisticsReturn {
  const { t } = useLanguage()
  ...
  const capacityUtilization = computed(() => {
    const upcomingWithCapacity = events.value.filter(event => 
      event.is_upcoming && event.capacity && event.current_registrations
    )
    
    if (upcomingWithCapacity.length === 0) return 0
    
    const totalCapacity = upcomingWithCapacity.reduce((sum, event) => sum + (event.capacity || 0), 0)
    const totalRegistered = upcomingWithCapacity.reduce((sum, event) => sum + (event.current_registrations || 0), 0)
