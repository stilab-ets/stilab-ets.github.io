...
export function useVacancies(): UseVacanciesReturn {
  ...
  const activeVacancies = computed(() => {
    ...
    return vacancies.value.filter(
      (vacancy) => vacancy.is_active && new Date(vacancy.deadline) > now
    );
  });
  ...
  const upcomingDeadlines = computed(() => {
    ...
    return activeVacancies.value
      .filter((vacancy) => new Date(vacancy.deadline) <= oneWeekFromNow)
      .sort(
        (a, b) =>
  ...
  const uniqueSupervisors = computed(() => {
    const supervisors = new Set(
      vacancies.value.map((vacancy) => vacancy.supervisor)
    );
