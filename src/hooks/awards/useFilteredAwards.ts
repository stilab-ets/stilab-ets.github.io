...
export function useFilteredAwards({
  awards,
}: UseFilteredAwardsOptions): UseFilteredAwardsReturn {
  // Filter state
  ...
  const filteredAwards = computed(() => {
    return awards.value.filter((award) => {
      const awardYear =
        award.year || new Date(award.date_received).getFullYear();
      const matchesYear =
        !selectedYear.value || awardYear.toString() === selectedYear.value;
      const matchesOrg =
        !selectedOrganization.value ||
        award.organization === selectedOrganization.value;
