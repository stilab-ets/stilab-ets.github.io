...
export function usePublicationFilters(
  ...
  const availableEntryTypes = computed(() => {
    ...
    publications.value.forEach((pub) => {
      if (
        ...
        typeof pub.publication_type === 'string'
      ) {
  ...
  const filteredPublications = computed(() => {
    ...
    return publications.value.filter((pub) => {
      ...
      const matchesYear =
        !selectedYear.value ||
        (pub.year && pub.year.toString() === selectedYear.value);

      // Type filter
      const matchesType =
        !selectedType.value || pub.publication_type === selectedType.value;
  ...
  const sortedPublications = computed(() => {
    if (
      !filteredPublications.value ||
      !Array.isArray(filteredPublications.value)
    ) {
      return [];
    }
