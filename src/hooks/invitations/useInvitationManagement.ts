...
export function useInvitationManagement() {
  ...
  const updateInvitation = async (
    ...
    try {
      ...
      const invitationIndex = invitations.value.findIndex(
        (inv) => inv.id === id
      );
  ...
  const invitationStats = computed(() => {
    ...
    const registered = invitations.value.filter(
      (inv) => inv.status === 'registered'
    ).length;
    const expired = invitations.value.filter(
      (inv) => inv.status === 'expired'
    ).length;
    const canceled = invitations.value.filter(
