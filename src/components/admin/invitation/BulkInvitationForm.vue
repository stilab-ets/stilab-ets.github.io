...
const isBulkFormValid = computed(() => {
  return validEmails.value.length > 0 && bulkForm.value.role !== '' && invalidEmails.value.length === 0
})
...
<template>
  <Card class="p-6">
    <div class="space-y-6">
      <!-- Header -->
      <div>
        <h3 class="text-lg font-medium text-gray-900">{{ t.form.bulkTitle }}</h3>
        <p class="mt-1 text-sm text-gray-600">{{ t.form.bulkSubtitle }}</p>
      </div>

      <!-- Bulk Results -->
      <div v-if="bulkResults.success.length > 0 || bulkResults.errors.length > 0" class="space-y-3">
        <!-- Success Results -->
        <div v-if="bulkResults.success.length > 0" class="bg-green-50 border border-green-200 rounded-md p-4">
          <div class="flex">
            ...
            <div class="ml-3">
              <h4 class="text-sm font-medium text-green-800">
                {{ bulkResults.success.length }} {{ t.success.bulkInvitationsSent }}
              </h4>
              <div class="mt-2 text-sm text-green-700">
                <ul class="list-disc list-inside space-y-1">
                  <li v-for="email in bulkResults.success" :key="email">{{ email }}</li>
                </ul>
              </div>
            </div>
          </div>
        ...
        <!-- Error Results -->
        <div v-if="bulkResults.errors.length > 0" class="bg-red-50 border border-red-200 rounded-md p-4">
          <div class="flex">
            <div class="flex-shrink-0">
            ...
            <div class="ml-3">
              ...
              <div class="mt-2 text-sm text-red-700">
                <ul class="list-disc list-inside space-y-1">
                  <li v-for="error in bulkResults.errors" :key="error">{{ error }}</li>
                </ul>
              </div>
            </div>
          </div>
      ...
      <form @submit.prevent="handleBulkSubmit" class="space-y-4">
        ...
        <!-- Email List Input -->
        <div>
          <label for="bulkEmails" class="block text-sm font-medium text-gray-700">
            {{ t.form.bulkEmails }}
            <span class="text-red-500">*</span>
          </label>
          <textarea
        ...
        <!-- Form Summary -->
        <div v-if="emailList.length > 0" class="text-sm text-gray-600 text-center">
