...
const validateForm = () => {
  ...
  if (
    formData.value.registration_url &&
    !validateURL(formData.value.registration_url)
  ) {
  ...
  >
    <form @submit.prevent="submitForm" class="space-y-8">
      ...
      <div>
        ...
        <p v-if="errors.domain" class="text-sm text-red-600">
          {{ errors.domain }}
        </p>
      </div>
      ...
      <div>
        ...
        <p v-if="errors.speaker" class="text-sm text-red-600">
          {{ errors.speaker }}
        </p>
      </div>
