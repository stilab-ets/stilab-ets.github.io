...
const isCapacityNearFull = computed((): boolean => {
  const current =
    props.eventData.current_registrations ||
    props.eventData.currentRegistrations;
  return !!(
    props.eventData.capacity &&
    current &&
...
const openRegistrationLink = (): void => {
  const url =
    props.eventData.registration_url || props.eventData.registrationUrl;
  if (url) {
    openUrl(url);
  }
};
  ...
  <Card variant="hover">
      ...
      <div class="flex-1">
        <div class="flex items-center mb-2">
          ...
          <span
            v-if="
              !isPast &&
              (eventData.registration_url || eventData.registrationUrl)
            "
            class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800"
          >
            {{
              getTranslation(
                'events.eventCard.registrationOpen',
                'Registration Open'
              )
            }}
          </span>
          <span
            v-if="!isPast && isCapacityNearFull"
    ...
    <div v-if="speakerName" class="mb-4">
      <p class="text-sm text-gray-600">
        <span class="font-medium"
          >{{ getTranslation('events.eventCard.speaker', 'Speaker') }}:</span
        >
        {{ speakerName }}
      </p>
    </div>
    ...
    <div
      v-if="
        !isPast &&
        eventData.capacity &&
        (eventData.current_registrations || eventData.currentRegistrations)
      "
      class="mb-4"
    >
      <div class="flex items-center justify-between text-sm text-gray-600 mb-2">
        <span>{{
          getTranslation('events.eventCard.registrations', 'Registrations')
        }}</span>
        <span
          >{{
            eventData.current_registrations || eventData.currentRegistrations
          }}
          / {{ eventData.capacity }}</span
        >
      </div>
      <div class="w-full bg-gray-200 rounded-full h-2">
        <div
          class="bg-[#08a4d4] h-2 rounded-full"
    ...
    <!-- Description -->
    <p v-if="eventData.description" class="text-gray-700 mb-4 leading-relaxed">
    ...
    <div
      v-if="
        !isPast && (eventData.registration_url || eventData.registrationUrl)
      "
      class="mt-4"
    >
      <Button
