...
const filters = computed(() => [
  {
    ...
    options: [
      ...
      ...uniqueTypes.value.map((type) => ({
        value: type,
        label:
          (t.value.vacancies.positionTypes as Record<string, string>)[type] ||
          type,
      })),
    ],
  },
...
const resultsText = computed(() => {
  ...
  if (count === 0)
    return `0 ${t.value.vacancies.results.offer} ${t.value.vacancies.results.available}`;
  if (count === 1)
    return `1 ${t.value.vacancies.results.offer} ${t.value.vacancies.results.available}`;
