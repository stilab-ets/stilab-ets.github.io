...
const submitForm = async () => {
  ...
  try {
    ...
    if (success) {
      successMessage.value = 'Research project created successfully! Redirecting...'
      formData.value = {
        title: '',
        start_date: '',
        end_date: '',
    ...
    <form @submit.prevent="submitForm" class="space-y-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          />
          <p v-if="errors.project_url" class="text-sm text-red-600">{{ errors.project_url }}</p>
        </div>
        <div>
          ...
          />
          <p v-if="errors.github_url" class="text-sm text-red-600">{{ errors.github_url }}</p>
        </div>
      </div>
