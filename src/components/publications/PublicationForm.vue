...
const generateId = () => {
  if (formData.value.citekey) {
    formData.value.id = formData.value.citekey
      .toLowerCase()
      .replace(/[^a-z0-9]/g, '');
  } else {
...
const validateForm = () => {
  ...
  if (!formData.value.entrytype) {
    errors.value.entrytype =
      t.value.forms.publication.validation.entryTypeRequired;
  }
  ...
  if (
    formData.value.year &&
    (formData.value.year < 0 || formData.value.year > 2147483647)
  ) {
...
const submitForm = async () => {
  ...
  try {
    ...
    if (success) {
      successMessage.value =
        'Publication created successfully! Redirecting to dashboard...';
      ...
      errors.value.general =
        error.value || t.value.forms.publication.errors.submitFailed;
  ...
  >
    ...
    <form @submit.prevent="submitForm" class="space-y-6">
      ...
      <div>
        ...
        >
          <option value="">
            {{ t.forms.publication.form.selectEntryType }}
          </option>
          <option v-for="type in entryTypes" :key="type" :value="type">
            {{
              t.forms.publication.entryTypes[
                type as keyof typeof t.forms.publication.entryTypes
              ]
            }}
          </option>
        </select>
      ...
      <div>
        ...
        <p class="mt-1 text-xs text-gray-500">
          ID will be auto-generated from citekey
        </p>
      </div>
      ...
      <div>
        ...
        />
        <p class="mt-1 text-xs text-gray-500">
          {{ t.forms.publication.form.authorsHelp }}
        </p>
      </div>
      ...
      <div class="p-4 bg-gray-50 border border-gray-200 rounded-lg">
        <p class="text-sm text-gray-600">
          <strong>Note:</strong> Publications are submitted for approval by
          default and will be reviewed before appearing publicly.
        </p>
      </div>
