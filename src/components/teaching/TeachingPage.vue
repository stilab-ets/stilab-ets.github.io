...
const getFullSemester = (semester: string, year: number) => {
  ...
  switch (semester) {
    case 'F':
      label = t.value.teaching.semesters.fall;
      break;
    case 'W':
      label = t.value.teaching.semesters.winter;
      break;
    case 'S':
      label = t.value.teaching.semesters.summer;
      break;
    default:
      label = semester;
  }
...
const availableSemesters = computed(() => {
  const semesters = courses.value.map((c) =>
    getFullSemester(c.semester, c.year)
  );
...
const uniqueInstructors = computed(() => {
  return [
    ...new Set(
      courses.value.map((c) => `${c.teacher.first_name} ${c.teacher.last_name}`)
    ),
  ];
});
...
const statistics = computed(() => [
  {
    ...
    label: t.value.teaching.statistics.coursesOffered,
  },
  {
    value: uniqueInstructors.value.length,
    label: t.value.teaching.statistics.instructors,
  },
