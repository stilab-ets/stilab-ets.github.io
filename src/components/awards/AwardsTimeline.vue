...
const getRecipientName = (award: ExtendedAward): string => {
  if (award.recipients && award.recipients.length > 0) {
    return award.recipients.map(r => `${r.first_name} ${r.last_name}`).join(', ')
  }
...
<template>
  <div class="space-y-6">
    <!-- Year Group -->
    <div v-for="yearGroup in groupedAwards" :key="yearGroup.year" class="relative bg-white rounded-xl p-4 shadow hover:shadow-md transition-shadow">
      <!-- Year Header -->
      <div class="flex items-center mb-2">
        <div class="flex items-center justify-center w-12 h-12 bg-[#08a4d4] rounded-full text-white font-bold text-lg">
          {{ yearGroup.year }}
        </div>
        <div class="ml-4">
      ...
      <div class="ml-12 grid grid-cols-1 gap-2">
        <div v-for="award in yearGroup.awards" :key="award.id" class="bg-white rounded-lg duration-200 p-6 relative">
          <!-- Award Card -->
          <div class="flex items-start w-full">
            ...
            <div class="flex-1">
              ...
              <div class="flex items-center mb-2">
                ...
                <span class="text-sm text-[#08a4d4] ml-1 font-medium">{{ getRecipientName(award) }}</span>
              </div>
              ...
              <div class="flex items-center justify-between">
                ...
                <span class="text-xs text-gray-500">{{ getAwardYear(award) }}</span>
              </div>
            </div>
          </div>

          <!-- Shine effect for recent awards -->
          <div v-if="getAwardYear(award) >= currentYear - 1" class="absolute top-2 right-2">
