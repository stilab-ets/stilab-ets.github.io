...
const visibleErrors = computed(() => 
  errors.value.slice(-props.maxErrors)
)
  ...
  <div v-if="hasErrors" class="fixed top-4 right-4 z-50 max-w-sm">
    <div class="space-y-2">
      ...
      >
        <div class="flex items-start space-x-3">
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-center justify-between">
              ...
              <button
                @click="removeError(errors.length - visibleErrors.length + index)"
        ...
        <div v-if="error.status && error.status >= 500" class="mt-3 flex space-x-2">
          <button
            @click="reload"
