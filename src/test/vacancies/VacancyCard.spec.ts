...
describe('VacancyCard.vue', () => {
  ...
  it('renders the requirements list', () => {
    const requirements = wrapper.findAll('[class*="requirements"] , [class*="flex items-center text-sm text-gray-600"]')
  ...
  it('renders the deadline and start date sections', () => {
    const deadlineSection = wrapper.findAll('svg').filter(svg => svg.attributes('viewBox') === '0 0 24 24')
